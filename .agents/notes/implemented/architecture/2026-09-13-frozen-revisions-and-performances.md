...
The [submission service](../../../../plugins/story-compiler/README.md) compiles a pinned draft before one SQLite revision transaction. It requires success by default; explicit permission allows script errors to produce a source-only revision. Multiple targets attach all results or none. Operational failures always abort. The final sequence check rejects intervening edits without recompiling or retrying.

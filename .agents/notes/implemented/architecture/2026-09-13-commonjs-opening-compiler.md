...
Native ESM would offer standard module semantics but couple the authored language to experimental VM module interfaces. Bundling or transpilation would add resolution and location-mapping behavior that this text-only compiler does not need. Re-evaluating source during initialization would make preview and runtime depend on compiler execution after an artifact had been accepted. Keeping only in-memory results would prevent restart recovery. Storing artifacts in revision bodies would mix mutable derived formats with immutable authored content.

## Consequences

The initial language has no asynchronous execution, host I/O or runtime functions. Future game mechanisms need explicit artifact and runtime changes. Worker and VM controls limit resource use without promising complete hostile-code isolation. Successful artifacts accumulate until explicitly cleared; they have integrity hashes, not signatures. Built-worker checks use plain Node, actual Cordis tests cover cleanup, and deterministic model/browser tests cover receipts and previews without real model calls. Engineering readiness remains separate from creative-use feedback.

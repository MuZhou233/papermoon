...
The schema has its own identity and version and rejects unsupported files. It contains no compiler, approval process, model/session binding or UI. The default Web launcher remains unchanged. Public API changes follow main-repository standards, while actual Cordis compatibility is checked separately against built host artifacts. Main checks do not require the submodule.

...
Current fixture recordings use the DSH format constant and current message structure. Product data migration, historical aliases and format-evolution tests remain excluded. DSH keeps responsibility for its own protocol versions and recovery. Story mode and the DeepSeek API Key chapter are unchanged.

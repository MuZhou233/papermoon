...
function setup() {
  const dir = mkdtempSync(join(tmpdir(), 'pm-editor-unit-')),
    storage = new PlaybookStorage({ path: join(dir, 'db') }),
    repo = new PlaybookRepository(storage)
  cleanups.push(() => {
    storage.close()
    rmSync(dir, { recursive: true, force: true })
  })
  const project = repo.createProject({ name: 'Project' }),
    playbook = repo.createPlaybook({
  ...
  const data = new Map<string, Backup>(),
    backups: Backups = {
      list: async (id) => [...data.values()].filter((v) => v.playbookId === id),
      put: async (value) => {
...
describe('editor interface', () => {
  it('validates requests before calling core and exposes no publication or metadata writes', async () => {
    ...
    for (const [method, payload] of [
      ...
      [
        'commit',
        { playbookId: playbook.id, expectedSequence: 0, description: '  ' },
      ],
      ['registerPublication', {}],
    ]) {
      expect(
    ...
    const diff = JSON.parse(
      JSON.stringify(
        await api.call('compare', {
          ...
          right: { kind: 'draft', playbookId: playbook.id },
        }),
      ),
    )
    expect(
  ...
  it('queries across projects with literal filters, pages, and descending histories', () => {
    const { repo, project, playbook } = setup()
    repo.createPlaybook({
      projectId: repo.createProject({ name: 'Other' }).id,
      name: 'Two',
      defaultLanguage: 'en',
    })
    expect(repo.queryPlaybooks({ limit: 1 }).next).toBeDefined()
    expect(repo.queryPlaybooks({ projectId: project.id }).items).toHaveLength(1)
    expect(repo.queryPlaybooks({ query: '%' }).items).toHaveLength(0)
    for (let i = 0; i < 3; i++)
      repo.commitRevision({
        playbookId: playbook.id,
        expectedSequence: i,
    ...
    const page = repo.listRevisions(playbook.id, { descending: true, limit: 2 })
    expect(page.items.map((v) => v.ordinal)).toEqual([3, 2])
    expect(
      repo
        .listRevisions(playbook.id, { descending: true, after: page.next })
        .items.map((v) => v.ordinal),
    ).toEqual([1])
  })
  it('restores and copies independent drafts without duplicating revision bodies', async () => {
    const { api, playbook, project } = setup()
    const first = await api.call('commit', {
      allowCompilationFailure: true,
      playbookId: playbook.id,
      expectedSequence: 0,
    ...
    await api.call('deletePlaybook', { playbookId: playbook.id })
    expect(
      (await api.call('history', { playbookId: copy.id })).items[0]?.revision.id,
    ...
    expect(
      (
        await api.call('snapshot', {
          ref: { kind: 'draft', playbookId: copy.id },
        })
...
describe('local drafts', () => {
  it('keeps edits typed while a save is in flight and does not clear their backup', async () => {
    const { api, base, backups, playbook } = setup()
    let release!: () => void
...
it('deep revision reads validate playbook membership independently of pagination', async () => {
  const { api, repo, playbook, project } = setup()
  ...
  const other = repo.createPlaybook({
    projectId: project.id,
    name: 'Other',
    defaultLanguage: 'en',
  })

...
test('explicit compilation saves artifacts and previews exact roles after refresh', async ({ page }) => {
  ...
  const playbook = await rpc(page, 'createPlaybook', { projectId: project.id, name: 'Opening', defaultLanguage: 'en' })
  ...
  await page.getByRole('button', { name: 'playbook.js', exact: true }).click()
  ...
  expect((await rpc(page, 'snapshot', { ref: { kind: 'draft', playbookId: playbook.id } })).draft.sequence).toBe(1)
  await panel.getByRole('button', { name: 'Compile', exact: true }).click()
  ...
  const committed = await rpc(page, 'commit', { playbookId: playbook.id, expectedSequence: 2, description: 'Opening context' })
  await goto(page, new URL('/#papermoon/' + playbook.id + '?tab=history&revision=' + committed.revision.id, server.url).href)
...
test('diagnostics navigate to source and text, and stale results cannot navigate', async ({ page }) => {
  ...
  await panel.getByRole('button', { name: 'playbook.js:2', exact: true }).click()
  await expect(page.locator('.cm-content[contenteditable=true]')).toBeFocused()
  await page.locator('.cm-content[contenteditable=true]').fill('module.exports={systemPrompt:require("@papermoon/playbook").t("missing"),messages:[]};')
  await expect(panel.getByRole('button', { name: 'playbook.js:2', exact: true })).toBeDisabled()
  await panel.getByRole('button', { name: 'Compile', exact: true }).click()
  await page.getByRole('dialog').getByRole('button', { name: 'Save and compile', exact: true }).click()
  ...
  await rpc(page, 'save', { playbookId: playbook.id, expectedSequence: 2, operations: [{ kind: 'set-translation', key: 'missing', language: 'en', text: '' }] })
  await page.getByRole('button', { name: 'Refresh', exact: true }).click()
...
test('manual program and text edits persist as immutable revisions', async ({
  ...
  await login(page)
  await page
    .getByRole('button', { name: 'New playbook', exact: true })
    .first()
    .click()
  await page
...
test('RPC authentication and cross-window conflicts preserve pending text', async ({
  ...
  const project = await rpc(page, 'createProject', {
    ...
    playbook = await rpc(page, 'createPlaybook', {
  ...
  await login(other)
  await goto(other, new URL('/#papermoon/' + playbook.id + '?tab=draft', server.url).href)
...
test('responsive overview and independent playbook copy', async ({ page }) => {
  await login(page)
  ...
  await rpc(page, 'deletePlaybook', { playbookId: playbook.id })
  const revisions = await rpc(page, 'history', { playbookId: copied.id })
  expect(revisions.items[0].revision.id).toBe(committed.revision.id)
  await reload(page)
  await page
    .getByRole('textbox', { name: 'Search playbooks' })
    .fill('Another direction')
  await expect(page.locator('.pm-playbook-card')).toHaveCount(1)
  await page.setViewportSize({ width: 520, height: 850 })
  await page.screenshot({
...
test('historical comparison, partial restoration and copy use saved revisions', async ({
  ...
  await goto(
    page,
    new URL(
      '/#papermoon/' + playbook.id + '?tab=history&revision=' + first.revision.id,
      server.url,
    ).href,
  )
  await page
  ...
  const restored = await rpc(page, 'snapshot', {
    ref: { kind: 'draft', playbookId: playbook.id },
  })
  ...
  await page.getByRole('button', { name: 'Copy playbook', exact: true }).click()
  await page
    .getByRole('textbox', { name: 'Name', exact: true })
    .fill('History copy')
  await page.getByRole('button', { name: 'Confirm', exact: true }).click()

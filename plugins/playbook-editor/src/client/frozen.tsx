...
export function FrozenCompilation({ api, playbookId, revisionId, t }: { api: Api; playbookId: string; revisionId: string; t: T }) {
  ...
  useEffect(() => {
    let alive = true
    void api.call('revisionCompilation', { playbookId, revisionId }).then(report => { if (alive) { setReport(report); setKey(report?.targets[0]?.attachmentKey ?? '') } }).catch(error => { if (alive) setError(String(error)) })
    return () => { alive = false }
  }, [api, playbookId, revisionId])
  useEffect(() => {
    setResult(undefined)
    if (!key) return
    let alive = true
    void api.call('revisionArtifact', { playbookId, revisionId, key }).then(result => { if (alive) { setResult(result); setError('') } }).catch(error => { if (alive) setError(String(error)) })
    return () => { alive = false }
  }, [api, playbookId, revisionId, key])
  return <section className="pm-compilation">
    ...
    {result && <p>{t(result.artifact.composition ? 'customComposition' : 'defaultComposition')}</p>}

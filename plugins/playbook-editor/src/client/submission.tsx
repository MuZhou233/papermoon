...
import type { PlaybookContent } from '@papermoon/playbook-core'
import type { SubmissionResult } from '@papermoon/playbook-compiler/service'
import type { Api } from './api.ts'
import type { T } from './locales.ts'
import { RevisionPicker } from './revision-picker.tsx'
export function SubmissionDialog({ api, playbookId, sequence, content, t, close, committed }: {
  api: Api; playbookId: string; sequence: number; content: PlaybookContent; t: T; close: () => void; committed: (result: Extract<SubmissionResult, { committed: true }>) => Promise<void>
}) {
  const [description, setDescription] = useState(''), [references, setReferences] = useState('')
  const [targets, setTargets] = useState([{ entry: 'playbook.js', language: content.texts.defaultLanguage }])
  ...
  const run = async () => {
    ...
    try {
      const result = await api.call('commit', { playbookId, expectedSequence: sequence, description, references: references.split('\n').filter(Boolean), targets, allowCompilationFailure: allow }, pending.signal)
  ...
  return <Modal open title={t('commit')} description={t('commitHint')} closeLabel={t('close')} onClose={() => { if (!busy) close() }} footer={<>
    ...
    <div className="pm-form">
      ...
      <fieldset disabled={busy} className="pm-targets"><legend>{t('compileTargets')}</legend>
        ...
        <Button disabled={busy} onClick={() => setTargets([...targets, { entry: 'playbook.js', language: content.texts.defaultLanguage }])}>{t('addTarget')}</Button>
      </fieldset>

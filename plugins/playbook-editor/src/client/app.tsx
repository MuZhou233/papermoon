  ...
  const create = () => {
    ...
    ask({
      ...
      submit: async (v) => {
        ...
        const playbook = await runtime.api.call('createPlaybook', {
          name: v.name!,
          projectId,
          defaultLanguage: v.language!,
        })
        go({ playbookId: playbook.id, tab: 'draft' })
      },
    })
  }
  return (
    <main className="pm-app pm-theme">
      <header className="pm-header" hidden={!!route.playbookId}>
            ...
            <div className="pm-card-bottom">
              <span>
                {playbook.latestOrdinal
                  ? t('latest') + ' ' + playbook.latestOrdinal
                  : t('noRevisions')}
              </span>
...
function Detail({
  ...
  const [info, setInfo] = useState<Results['playbook']>(),
  ...
  useEffect(() => {
    let alive = true
    Promise.all([
      runtime.api.call('playbook', { playbookId: id }),
      runtime.api.call('snapshot', { ref: { kind: 'draft', playbookId: id } }),
    ])
      .then(([info, snapshot]) => {
        if (!alive) return
  ...
  return (
    <>
      ...
      <Tabs
        ...
        onChange={(tab) => go({ playbookId: id, tab: tab as Route['tab'] })}
      />
      {route.tab === 'settings' ? (
        <section className="pm-settings">
          ...
          <Button
            ...
            onClick={() =>
              ask({
                title: t('renamePlaybook'),
                fields: [
                  { id: 'name', label: t('name'), value: info.playbook.name },
                ],
                submit: async (v) => {
                  await runtime.api.call('renamePlaybook', {
                    playbookId: id,
                    name: v.name!,
                  })
                  setVersion((v) => v + 1)
          ...
          <div className="pm-danger-zone">
            <p>{t('deletePlaybookHint')}</p>
            <Button
              className="pm-danger"
              onClick={() =>
                ask({
                  title: t('deletePlaybook'),
                  description: t('deletePlaybookHint'),
                  danger: true,
                  submit: async () => {
                    await runtime.api.call('deletePlaybook', { playbookId: id })
...
function Workspace({
  ...
  const copy = (revisionId?: string) => {
    try {
      ...
      ask({
        ...
        submit: async (v) => {
          requireSaved()
          const playbook = await runtime.api.call('copy', {
            sourcePlaybookId: info.id,
            source: revisionId
  ...
  return (
    <>
                ...
                ask({
                  ...
                  submit: async () => {
                    const result = await runtime.api.call('snapshot', {
                      ref: { kind: 'draft', playbookId: info.id },
                    })
      ...
      {submission && <SubmissionDialog api={runtime.api} playbookId={info.id} sequence={base.draft.sequence} content={base.content ? fromDTO(base.content) : state.content} t={t} close={() => setSubmission(false)} committed={async result => { await editor.refresh(); setVersion(v => v + 1); go({ playbookId: info.id, tab: 'history', revisionId: result.revision.id }) }} />}
      {route.tab === 'draft' ? (
        <>
          ...
          <CompilationPanel
            ...
            save={async () => {
              await editor.save()
              requireSaved()
              return { playbookId: info.id, ref: { kind: 'draft', sequence: editor.getSnapshot().base.draft.sequence } }
            }}
            locate={setTarget}
            verify={async () => {
...
function RevisionBrowser({
  ...
  useEffect(() => {
    let alive = true
    runtime.api
      .call('history', { playbookId: route.playbookId! })
      .then((page) => {
        if (alive) {
          setRows(page.items)
          setNext(page.next)
  ...
  }, [route.revisionId, route.tab, route.playbookId, rows, runtime])
  return (
        ...
        {next && (
          <Button
            ...
            onClick={() => {
              setBusy(true)
              void runtime.api
                .call('history', { playbookId: route.playbookId!, after: next })
                .then((page) => {
                  setRows((before) => [...before, ...page.items])
                  setNext(page.next)
                })
          ...
          <>
            <div className="pm-revision-meta">
              ...
              <div className="pm-muted">
                ...
                {snapshot.revision.source.playbookName}
                <br />
                {t('base')}:{' '}
              ...
              <div className="pm-actions">
                <Button variant="primary" disabled={!snapshot.revision.attachments.items.length} onClick={() => window.dispatchEvent(new CustomEvent('papermoon:performance', { detail: { playbookId: route.playbookId, revisionId: snapshot.revision.id } }))}>{t('startPerformance')}</Button>
                <Button
                  onClick={() =>
                    setComparison({
                      left: {
                      ...
                      right: {
                        kind: 'draft',
                        playbookId: route.playbookId as PlaybookId,
                      },
                    })
                  }
                >
            ...
            <FrozenCompilation key={snapshot.revision.id} api={runtime.api} playbookId={route.playbookId!} revisionId={snapshot.revision.id} t={t} />
            {comparison ? (
              <Comparison

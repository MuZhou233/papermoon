...
import type { CompilationReceipt } from '@papermoon/playbook-compiler/service'
import type { Diagnostic } from '@papermoon/playbook-compiler/types'
...
export function CompilationPanel({ api, source, content, t, ask, dirty = false, blocked = false, save, locate, verify }: Props) {
  const identity = source.playbookId + '/draft'
  const key = 'papermoon.compile/' + identity
  ...
  const [entry, setEntry] = useState(settings.entry ?? 'playbook.js')

...
export function jsonValue(value: unknown, location: string): JsonValue {
  try { return JSON.parse(encodeJson(value)) as JsonValue } catch (cause) {
    throw new PlaybookError('invalid-content', 'expected a JSON-storable value', location, { cause })
  }
}
...
export function decodeRecord(recordKey: string, raw: JsonValue): ContentRecord {
  ...
  if (recordKey === 'content') {
    ...
    return { kind: 'settings', value: { format: CONTENT_FORMAT, defaultLanguage,
...
export function decodeContent(records: Content): PlaybookContent {
  try {
    ...
    if (error instanceof PlaybookError && error.code !== 'invalid-content') throw new PlaybookError('invalid-content', error.message, error.location, { cause: error })
    throw error
  }
}

...
export const compositionTrajectory = {
  ...
  buildViewNode(context:Context){const first=context.matches[0];if(!first)return null;return{key:context.key,id:context.id,kind:'papermoon-composition-record',target:'trajectory',anchorSeq:first.event.seq,location:first.location,visibility:'visible',independent:true,data:{kind:'node',node:{kind:'context',seq:first.event.seq,time:first.event.time,content:[{type:'text',text:JSON.stringify(first.event.data,null,2)}],source:{kind:'plugin',plugin:'papermoon.context'},provenance:{role:'inject',label:'PaperMoon'},form:'snapshot'}}}},
}

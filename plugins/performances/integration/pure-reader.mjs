...
const guard = registerHooks({ resolve(specifier, context, next) {
  if (['node:worker_threads', 'node:sqlite'].includes(specifier) || specifier.endsWith('/story-compiler/lib/index.js') || specifier === '@papermoon/story-compiler' || specifier === '@papermoon/story-compiler/service') throw new Error('performance imported execution service: ' + specifier)
...
try {
  ...
  const artifact = createArtifact('0'.repeat(64), { entry: 'story.js', language: 'en', limits: { inputBytes: 1, modules: 1, outputBytes: 1000, executionMs: 1, totalMs: 1, concurrency: 1, memoryMb: 1 } }, { systemPrompt: 'Frozen', messages: [] })

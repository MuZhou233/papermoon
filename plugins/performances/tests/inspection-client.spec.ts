...
test('late inspection responses cannot replace a newer selection or select runtime history', async () => {
  ...
  const state={sessionId:'session',view:{fixed:{artifact:{composition:{}}},worldline:{nodes,selected:'a',version:1,count:3,legacy:false}}}
  ...
  inspection.select('a');state.sessionId='legacy';state.view.worldline.legacy=true;inspection.observe()

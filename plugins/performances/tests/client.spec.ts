...
test('refreshes action commits without list activity, follows the selected session and releases subscriptions', async () => {
  ...
  let count = 0
  const call = vi.fn(async () => ({ ok: true as const, value: { fixed: {}, runtime: { state: { count }, actions: [] } } }))
  const host: ClientHost = {
    chatPresentation: { preserveReplies: () => () => {} },
    connection: { rpc: { call } }, sessions: { list, refresh: async () => {}, open: () => {}, binding: id => ({ eventSource: id === 'a' ? a : b }) },
    uiAgentPreset: { store: mode, load: async () => {} }, conversation: { blocks: { set: () => {} } }, layout: { selectPanel: () => {} },

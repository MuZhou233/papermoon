...
test('accepted writer navigation opens the overview and preserves unsaved source', async ({ page }) => {
  ...
  const script = await storyApi('createScript', {
    projectId: project.id, name: 'Unfinished script', defaultLanguage: 'en',
  })
  await storyApi('save', {
    scriptId: script.id, expectedSequence: 0,
    operations: [{ kind: 'create-file', path: 'main.js', source: 'saved source' }],
  })
  await page.evaluate((id) => { location.hash = '#papermoon/' + id + '?tab=draft' }, script.id)
  ...
  await expect.poll(() => new URL(page.url()).hash).toBe('#papermoon/' + script.id + '?tab=draft')
  await page.getByRole('button', { name: 'Writer management', exact: true }).click()
  await page.locator('[data-prompt-message=system]').click()

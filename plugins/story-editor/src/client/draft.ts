...
const identity = (content: StoryContent) =>
...
export class DraftEditor {
  ...
  async refresh() {
    const result = await this.api.call('snapshot', {
      ref: { kind: 'draft', scriptId: this.state.base.draft.scriptId },
    })
  ...
  async save() {
    ...
    try {
      const result = await this.api.call('save', {
        scriptId: before.draft.scriptId,
      ...
      try {
        const latest = await this.api.call('snapshot', {
          ref: { kind: 'draft', scriptId: before.draft.scriptId },
        })

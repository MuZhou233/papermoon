...
export const source = `const {t}=require('@papermoon/playbook');
...
test('rolls back thrown errors, validates inputs and outputs, and never coerces strings', async () => {
  ...
  expect(await runtime.invoke(artifact, state, 'increment', { amount: 3 })).toMatchObject({ ok: false, diagnostics: [{ message: 'limit exceeded', location: { path: 'playbook.js' } }] })
...
test('runs frozen dependencies and exact-language text without reading current content', async () => {
  const input = applyOperations(content(`const factory=require('./functions.js'); module.exports={systemPrompt:'',messages:[],functions:[factory]};`), [
    { kind: 'create-file', path: 'functions.js', source: `const {t}=require('@papermoon/playbook');
...
return function message(fail) { const text=t('message'); if(fail) throw new Error(text); return text; }
  ...
  expect(Object.keys(compiled.artifact.program.files).sort()).toEqual(['functions.js', 'playbook.js'])
  const runtime = new PlaybookRuntime(); runtimes.push(runtime)
...
/** @typedef {{groups: Second[]}} Third */`
  ...
  expect(duplicate).toMatchObject({ok:false,diagnostics:[{message:'ambiguous typedef First',location:{path:'playbook.js',line:8}}]})
})
...
test.each(['object','Record<number, string>','Record<string, unknown>','{[key:number]: string}','Missing'])('locates unsupported types in the annotation: %s', async type => {
  ...
  expect(result).toMatchObject({ok:false,diagnostics:[{code:'invalid-jsdoc',location:{path:'playbook.js',line:4}}]})
})

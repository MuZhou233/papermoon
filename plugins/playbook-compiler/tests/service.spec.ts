...
import { createPlaybookTools } from '@papermoon/playbook-tools'
import { PlaybookObservations } from '../../playbook-tools/src/observations.ts'
...
async function fixture() {
  ...
  const source = (sequence = 1) => ({ playbookId: script.id, ref: { kind: 'draft' as const, sequence } })
...
describe('saved compilation', () => {
  it('compiles the program help example through the bound tools', async () => {
    ...
    const example = await execute('playbook_help', { topic: 'program' }) as { topic: string; text: string }
    const block = /```json\n([\s\S]*?)\n```/.exec(example.text)
    if (!block) throw new Error('program help has no executable example')
    expect(await execute('playbook_program_edit', JSON.parse(block[1]!))).toMatchObject({ sequence: 1 })
    expect(await execute('playbook_compile', { ref: { kind: 'draft', sequence: 1 } })).toMatchObject({ ok: true, context: { systemPrompt: '', messages: [] } })
  })
  it('keeps the snapshot fixed during edits and restores a matching artifact without executing', async () => {
    ...
    f.repository.editDraft({ playbookId: f.script.id, expectedSequence: 1, operations: [{ kind: 'set-translation', key: 'opening', language: 'en', text: 'Later' }] })
    ...
    expect(f.repository.readSnapshot({ kind: 'draft', playbookId: f.script.id }).draft.sequence).toBe(2)
    ...
    f.repository.deletePlaybook(f.script.id)
  ...
  it('freezes compilation with a revision independently of the preview directory', async () => {
    ...
    const result = await f.service.submit({ playbookId: f.script.id, expectedSequence: 1, description: 'Opening' })
    ...
    expect(f.repository.listPublications({ playbookId: f.script.id }).items).toEqual([])
    expect(f.repository.readSnapshot({ kind: 'draft', playbookId: f.script.id }).draft.sequence).toBe(2)
  })
  it('requires explicit permission for script errors and never attaches a partial target set', async () => {
    const f = await fixture()
    const input = { playbookId: f.script.id, expectedSequence: 1, description: 'Incomplete', targets: [{ entry: 'playbook.js' }, { entry: 'missing.js' }] }
  ...
  it('attaches successful results even when failure is permitted and preserves target order', async () => {
    const f = await fixture()
    f.repository.editDraft({ playbookId: f.script.id, expectedSequence: 1, operations: [{ kind: 'create-file', path: 'other.js', source: 'module.exports={systemPrompt:"Other",messages:[]}' }] })
    const result = await f.service.submit({ playbookId: f.script.id, expectedSequence: 2, description: 'Two', allowCompilationFailure: true, targets: [{ entry: 'other.js' }, { entry: 'playbook.js' }] })
    if (!result.committed) throw new Error('fixture submission failed')
  ...
  it('does not commit on cancellation, invalid targets, concurrent edits or attachment limits', async () => {
    const f = await fixture(), input = { playbookId: f.script.id, expectedSequence: 1, description: 'Opening', allowCompilationFailure: true }
    expect(() => f.service.submit({ ...input, targets: [] })).toThrow()
    expect(() => f.service.submit({ ...input, targets: [{}, { entry: 'playbook.js', language: 'en' }] })).toThrow()
    ...
    f.repository.editDraft({ playbookId: f.script.id, expectedSequence: 1, operations: [{ kind: 'set-translation', key: 'opening', language: 'en', text: 'Changed' }] })
  ...
  it('exposes async compilation only with its service and never grants observation rights', async () => {
    const f = await fixture(), observations = new PlaybookObservations()
    expect(createPlaybookTools(f.repository, f.script.id).some(t => t.name === 'playbook_compile')).toBe(false)
    const tools = createPlaybookTools(f.repository, f.script.id, observations, f.service)
    const execute = (name: string, args: unknown, signal = new AbortController().signal) => tools.find(t => t.name === name)!.execute(args, { signal })
    expect(await execute('playbook_compile', { ref: { kind: 'draft', sequence: 1 } })).toMatchObject({ ok: true, context: { systemPrompt: 'First' } })
    await expect(execute('playbook_program_edit', { operations: [{ kind: 'replace-file', path: 'playbook.js', source: '' }] })).rejects.toThrow()
    const controller = new AbortController(); controller.abort()
    await expect(execute('playbook_compile', { ref: { kind: 'draft', sequence: 1 } }, controller.signal)).rejects.toThrow()
    expect(f.repository.readSnapshot({ kind: 'draft', playbookId: f.script.id }).draft.sequence).toBe(1)
  })
})

it('does not permit service failures to create an uncompiled revision', async () => {
  const f = await fixture()
  const input = { playbookId: f.script.id, expectedSequence: 1, description: 'Stopped', allowCompilationFailure: true }
...
module.exports={systemPrompt:'',messages:[],state:{initial:{value:0},schema:{type:'object',properties:{value:{type:'number'}},required:['value'],additionalProperties:false}},functions:[factory]};`
  f.repository.editDraft({ playbookId: f.script.id, expectedSequence: 1, operations: [{ kind: 'replace-file', path: 'playbook.js', source: program }] })
  const pending = f.service.simulate(f.source(2), [{ name: 'add', args: { amount: 3 } }, { name: 'add', args: { amount: -1 } }, { name: 'add', args: { amount: 2 } }])
  f.repository.editDraft({ playbookId: f.script.id, expectedSequence: 2, operations: [{ kind: 'replace-file', path: 'playbook.js', source: '' }] })
  ...
  expect(f.repository.readSnapshot({ kind: 'draft', playbookId: f.script.id }).draft.sequence).toBe(3)
  expect(f.repository.listRevisions(f.script.id).items).toHaveLength(0)
  expect(await readdir(f.directory)).not.toContain('compiled')
  const tools = createPlaybookTools(f.repository, f.script.id, undefined, f.service)
  expect(await tools.find(t => t.name === 'playbook_simulate')!.execute({ calls: [] }, { signal: new AbortController().signal })).toMatchObject({ ok: false })
})

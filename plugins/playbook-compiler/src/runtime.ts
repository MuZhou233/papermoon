...
export function compilationKey(sourceHash: string, options: ResolvedOptions): string {
  return digest({ sourceHash, options, compiler: 'papermoon.playbook.commonjs' })
}
function freeze<T>(value: T): T {
  if (value && typeof value === 'object') { for (const child of Object.values(value)) freeze(child); Object.freeze(value) }
  return value
}
/** Deserialize a complete record; invalid or modified data never triggers recompilation. */

...
function syntax(source: string, path: string) {
  ...
  while (pending.length) {
    ...
    if (['ImportDeclaration', 'ExportNamedDeclaration', 'ExportDefaultDeclaration', 'ExportAllDeclaration'].includes(String(node.type)) ||
      ...
      fail('unsupported-syntax', 'parse', 'ES module syntax is not supported. Playbook programs use CommonJS require() and module.exports.', { path, line: loc.start.line - 1, column: loc.start.column + 1 })
    }
...
function diagnose(error: unknown, path?: string): Diagnostic {
  ...
  const frame = typeof e?.stack === 'string' ? /playbook:([^\n]*?):(\d+)(?::(\d+))?/.exec(e.stack) : null
  ...
  function load(path: string): unknown {
    ...
    const bridge = (specifier: unknown) => {
      try {
        ...
        if (specifier === '@papermoon/playbook') return { value: bootstrap.api }
        if (typeof specifier !== 'string' || !/^(\.\/|\.\.\/).*\.js$/.test(specifier) || specifier.includes('\\') || specifier.includes('\0'))
          fail('module-forbidden', 'load', 'require expects a relative .js path or @papermoon/playbook', { path })

...
import type { PlaybookRepository, PlaybookCommitResult } from '@papermoon/playbook-core/repository'
import { encodeContent, type PlaybookId, type ResolvedRef, type JsonObject, type JsonValue } from '@papermoon/playbook-core'
import { PlaybookRuntime } from './execution.ts'
import { PlaybookCompiler, prepare, resolveOptions } from './index.ts'
...
export interface CompilationSource { playbookId: PlaybookId; ref: { kind: 'draft'; sequence: number } }
export interface SubmissionInput {
  playbookId: PlaybookId; expectedSequence: number; description: string; metadata?: JsonObject; historyMetadata?: JsonObject
  references?: readonly import('@papermoon/playbook-core').RevisionId[]
...
export type SubmissionResult = { committed: false; compilation: import('./revisions.ts').RevisionCompilation } |
  { committed: true; revision: PlaybookCommitResult['revision']; entry: PlaybookCommitResult['entry']; draft: PlaybookCommitResult['draft']; compilation: import('./revisions.ts').RevisionCompilation }
...
export class CompilationService {
  ...
  private readonly runtime = new PlaybookRuntime()
  private readonly jobs = new Set<Promise<unknown>>()
  private readonly controllers = new Set<AbortController>()
  private closed = false
  constructor(private readonly repository: PlaybookRepository, readonly store: ArtifactStore, readonly defaults: CompileOptions = {}, readonly attachmentBytes = 16 * 1024 * 1024) {
  ...
  private snapshot(source: CompilationSource) {
    ...
    this.repository.getPlaybook(source.playbookId)
    ...
    return this.repository.readSnapshot({ kind: 'draft', playbookId: source.playbookId, sequence: source.ref.sequence })
  }
  ...
  submit(input: SubmissionInput, signal?: AbortSignal): Promise<SubmissionResult> {
    ...
    const work = (async (): Promise<SubmissionResult> => {
      ...
      const compilation: import('./revisions.ts').RevisionCompilation = {
        format: 'papermoon.compilation', status: successful ? 'success' : 'failed', sourceHash,
      ...
      controller.signal.throwIfAborted()
      const { revision, entry, draft } = this.repository.commitRevision({ playbookId: input.playbookId, expectedSequence: input.expectedSequence,

...
import type { PlaybookId, RevisionId } from '@papermoon/playbook-core'
...
export function revisionCompilation(repository: PlaybookRepository, playbookId: PlaybookId, revisionId: RevisionId): RevisionCompilation | null {
  const revision = repository.getHistoryEntry(playbookId, revisionId).revision
  const raw = revision.attachments.metadata.compilation
  if (raw === undefined) return null
  const result = raw as unknown as RevisionCompilation
  if (!result || result.format !== 'papermoon.compilation' ||
...
export class RevisionArtifacts {
  ...
  describe(playbookId: PlaybookId, revisionId: RevisionId) { return revisionCompilation(this.repository, playbookId, revisionId) }
  read(playbookId: PlaybookId, revisionId: RevisionId, key: string): Artifact {
    const report = this.describe(playbookId, revisionId)
  ...
  preview(playbookId: PlaybookId, revisionId: RevisionId, key: string) { return initialize(this.read(playbookId, revisionId, key)) }
}

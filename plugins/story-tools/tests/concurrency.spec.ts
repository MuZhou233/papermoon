...
import type { ContentOperation } from '@papermoon/story-core'
...
function fixture() {
  ...
  const repository = new StoryRepository(storage), project = repository.createProject({ name: 'Concurrent' })
  const script = repository.createScript({ projectId: project.id, name: 'Shared', defaultLanguage: 'en' })
  const snapshot = () => repository.readSnapshot({ kind: 'draft', scriptId: script.id })
  ...
  return { repository, storage, script, snapshot, session, writePeer, rawConnection }
}
...
test('sessions edit separate files and translations without refreshing a shared sequence', async () => {
  ...
  await a('story_program_read', { path: 'a.js' }); await b('story_program_read', { path: 'b.js' })
  await a('story_text_read', { key: 'opening', language: 'en' }); await b('story_text_read', { key: 'opening', language: 'zh-CN' })
  const receipts = await Promise.all([
    a('story_program_edit', replace('a.js', 'A')),
    b('story_text_edit', translate('zh-CN', '新内容')),
    b('story_program_edit', replace('b.js', 'B')),
    a('story_text_edit', translate('en', 'New content')),
  ])
  ...
  await expect(a('story_program_edit', replace('unread.js', 'unseen'))).rejects.toMatchObject({ code: 'not-observed' })
})

test('a changed observed object rejects the entire batch while unrelated human edits remain', async () => {
  const f = fixture(), a = f.session(), b = f.session()
  await a('story_program_read', { path: 'a.js' }); await b('story_program_read', { path: 'a.js' })
  f.writePeer([{ kind: 'replace-file', path: 'b.js', source: 'Human edit' }])
  await a('story_program_edit', replace('a.js', 'A'))
  const before = f.snapshot()
  await expect(b('story_program_edit', { operations: [{ kind: 'create-file', path: 'candidate.js', source: '' }, ...replace('a.js', 'B').operations] })).rejects.toMatchObject({ code: 'observation-stale' })
...
test('a competing translation between calculation and save is preserved on retry without granting whole-entry authority', async () => {
  ...
  await call('story_text_read', { key: 'opening' })
  ...
  await expect(call('story_text_edit', translate('en', 'Hello again'))).resolves.toMatchObject({ sequence: 3 })
  ...
  await expect(call('story_text_edit', { operations: [{ kind: 'rename-text', key: 'opening', to: 'renamed' }] })).rejects.toMatchObject({ code: 'observation-stale' })
  await expect(call('story_text_edit', translate('zh-CN', '未读修改'))).rejects.toMatchObject({ code: 'observation-stale' })
  await call('story_text_edit', translate('en', 'Own saved text remains observed'))
})

test('retry keeps the original file observation and refuses to recalculate over another author', async () => {
  const f = fixture(), call = f.session()
  await call('story_program_read', { path: 'a.js' })
  const write = f.storage.writeDraft.bind(f.storage)
  const attempts = vi.spyOn(f.storage, 'writeDraft').mockImplementationOnce(input => {
    f.writePeer([{ kind: 'replace-file', path: 'a.js', source: 'Other author' }])
    return write(input)
  })
  await expect(call('story_program_edit', { operations: [{ kind: 'replace-text', path: 'a.js', oldText: 'original', newText: 'Candidate' }] })).rejects.toMatchObject({ code: 'observation-stale' })
...
test('contention stops after three rejected writes and does not advance observations', async () => {
  ...
  await expect(call('story_program_edit', replace('a.js', 'Candidate'))).rejects.toMatchObject({ code: 'write-contention' })
  ...
  await call('story_program_edit', replace('a.js', 'After contention'))
})
...
test('cancellation between rejected save attempts leaves candidate content and observations uncommitted', async () => {
  ...
  await expect(call('story_program_edit', { operations: [{ kind: 'create-file', path: 'new.js', source: 'Candidate' }] }, abort.signal)).rejects.toThrow()
  expect(attempts).toHaveBeenCalledTimes(1)
  expect(f.snapshot().content.program.files.has('new.js')).toBe(false)
  f.writePeer([{ kind: 'create-file', path: 'new.js', source: 'Candidate' }])
  await expect(call('story_program_edit', replace('new.js', 'Changed'))).rejects.toMatchObject({ code: 'not-observed' })
})
...
test('SQL failure rolls back every file and a busy database is not retried', async () => {
  ...
  await call('story_program_read', { path: 'a.js' }); await call('story_program_read', { path: 'b.js' })
  ...
  await expect(call('story_program_edit', { operations: [...replace('a.js', 'Candidate').operations, ...replace('b.js', 'Candidate').operations] })).rejects.toMatchObject({ code: 'database-error' })
  expect(f.snapshot()).toEqual(before); expect(attempts).toHaveBeenCalledTimes(1)
  db.exec('DROP TRIGGER fail_second; BEGIN IMMEDIATE')
  try { await expect(call('story_program_edit', replace('a.js', 'Busy'))).rejects.toMatchObject({ code: 'busy' }) }
  finally { db.exec('ROLLBACK') }
  expect(attempts).toHaveBeenCalledTimes(2); expect(f.snapshot()).toEqual(before)
  await call('story_program_edit', replace('a.js', 'Saved after rollback'))
})
...
test('receipts and observations retain their committed snapshot when a later writer saves before the reply', async () => {
  ...
  await expect(call('story_program_edit', replace('a.js', 'My committed text'))).resolves.toMatchObject({ sequence: 2 })
  expect(f.snapshot().draft.sequence).toBe(3)
  await expect(call('story_program_edit', replace('a.js', 'Next edit'))).rejects.toMatchObject({ code: 'observation-stale' })
})
...
test('creation and path constraints reject collisions after a competing save', async () => {
  ...
  await expect(call('story_program_edit', { operations: [{ kind: 'create-file', path: 'new.js', source: 'Candidate' }] })).rejects.toMatchObject({ code: 'already-exists' })
  const before = f.snapshot()
  await expect(call('story_program_edit', { operations: [{ kind: 'create-file', path: 'a.js/child', source: '' }] })).rejects.toThrow()
  expect(f.snapshot()).toEqual(before)
})

test('default-language observations are independent of metadata, while language deletion pins the whole batch', async () => {
  const f = fixture(), a = f.session(), b = f.session()
  await a('story_status', {}); await b('story_status', {})
  await a('story_text_edit', { operations: [{ kind: 'set-default-language', language: 'zh-CN' }] })
  await expect(b('story_text_edit', { operations: [{ kind: 'set-default-language', language: 'en' }] })).rejects.toMatchObject({ code: 'observation-stale' })
  await b('story_text_edit', { operations: [{ kind: 'set-metadata', target: { kind: 'catalog' }, metadata: { label: 'Shared catalog' } }] })
  const before = f.snapshot()
  await expect(a('story_text_edit', { operations: [{ kind: 'delete-language', language: 'en', expectedSequence: 2 }] })).rejects.toMatchObject({ code: 'conflict' })
  expect(f.snapshot()).toEqual(before)
  await a('story_text_edit', { operations: [{ kind: 'delete-language', language: 'en', expectedSequence: 3 }] })
  expect(f.snapshot().content.texts.entries.get('opening')!.translations.has('en')).toBe(false)
  expect(schemas.story_text_edit.safeParse({ operations: [{ kind: 'delete-language', language: 'zh-CN' }] }).success).toBe(false)
})

test('revision commits and restoration still reject a changed complete snapshot', async () => {
  const f = fixture(), call = f.session()
  const revision = f.repository.commitRevision({ scriptId: f.script.id, expectedSequence: 1, description: 'Before edits' }).revision
  f.writePeer([{ kind: 'replace-file', path: 'a.js', source: 'New' }])
  await expect(call('story_commit', { expectedSequence: 2, description: 'Stale' })).rejects.toMatchObject({ code: 'conflict' })
  await expect(call('story_restore', { expectedSequence: 2, revisionId: revision.id, selection: { kind: 'all' } })).rejects.toMatchObject({ code: 'conflict' })
  expect(f.repository.listRevisions(f.script.id).items).toHaveLength(1)
...
test('an observed translation removed by another writer is not silently recreated', async () => {
  ...
  const before = f.snapshot()
  await expect(call('story_text_edit', translate('en', 'Stale replacement'))).rejects.toMatchObject({ code: 'observation-stale' })
  expect(f.snapshot()).toEqual(before)
  await expect(call('story_text_read', { key: 'opening', language: 'en' })).resolves.toMatchObject({ result: { kind: 'missing', reason: 'translation' } })
  await call('story_text_edit', translate('en', 'Explicit recreation'))
  f.writePeer([{ kind: 'delete-translation', key: 'opening', language: 'en' }])
  await call('story_text_read', { key: 'opening' })
  await call('story_text_edit', translate('en', 'After complete entry read'))
})

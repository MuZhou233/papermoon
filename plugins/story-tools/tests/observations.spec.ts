...
test('status does not refresh a stale file observation', async () => {
  ...
  await expect(call('story_program_edit', { operations: [{ kind: 'delete-file', path: 'main.js' }] })).rejects.toThrow(/observation-stale/)
  await call('story_program_read', { path: 'main.js' })
  await call('story_program_edit', { operations: [{ kind: 'delete-file', path: 'main.js' }] })
})
test('failed and canceled batches grant no observations and leave no partial files', async () => {
  ...
  await expect(call('story_program_edit', { operations })).rejects.toThrow(/not-observed/)
  ...
  await expect(call('story_program_edit', { operations: [operations[1]] })).rejects.toThrow(/not-observed/)
  await call('story_program_edit', { operations: [{ kind: 'create-file', path: 'new.js', source: '' }, { kind: 'replace-file', path: 'new.js', source: 'new' }] })
})
test('translation reads grant only the returned language, complete entry reads grant entry edits', async () => {
  ...
  await call('story_text_edit', { operations: [{ kind: 'set-translation', key: 'welcome', language: 'en', text: 'Hi' }] })
  await expect(call('story_text_edit', { operations: [{ kind: 'delete-text', key: 'welcome' }] })).rejects.toThrow(/not-observed/)
  await call('story_text_read', { key: 'welcome' })
  await call('story_text_edit', { operations: [{ kind: 'set-translation', key: 'welcome', language: 'en', text: 'Hello again' }] })
  await call('story_text_edit', { operations: [{ kind: 'rename-text', key: 'welcome', to: 'greeting' }] })
})
test('historical reads do not grant draft authority, explicit restore invalidates observations', async () => {
  ...
  await expect(call('story_program_edit', { operations: [{ kind: 'delete-file', path: 'main.js' }] })).rejects.toThrow(/not-observed/)
  await call('story_program_read', { path: 'main.js' })

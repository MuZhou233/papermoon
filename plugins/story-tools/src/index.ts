...
export function createStoryTools(
  ...
  function reference(
    ...
    return ref?.kind === 'revision'
      ? { kind: 'revision', revisionId: retained(ref.revisionId) }
      : {
          kind: 'draft',
          scriptId,
  ...
  function run(name: ToolName, raw: unknown): unknown {
    ...
    switch (name) {
      ...
      case 'story_commit': {
        ...
        const result = repository.commitRevision({
          ...
          historyMetadata: a.entryMetadata as
            | import('@papermoon/story-core').JsonObject
            | undefined,
          references: a.references?.map(retained),
        })
      ...
      case 'story_diff': {
        const a = schemas[name].parse(raw)
        return repository.compare(reference(a.left), reference(a.right), {
          ...a,
  ...
  return toolCatalog().map((descriptor) => ({
    ...
    async execute(
      ...
      try {
        return json(run(descriptor.name, args))
      } catch (error) {

...
import type { ContentPageOptions, ContentPage, StoryContent, TranslationLookup, TextSearch, SearchMatch, ProgramMatch, TextMatch } from './model.ts'
...
export function readFile(content: StoryContent, path: string) { return requireItem(content.program.files, path, 'program/' + path) }
export function readText(content: StoryContent, key: string) { return requireItem(content.texts.entries, key, 'text/' + key) }
export function readLanguage(content: StoryContent, language: string) { return requireItem(content.texts.languages, language, 'language/' + language) }
export function listFiles(content: StoryContent, options: ContentPageOptions = {}) { return pageItems(content.program.files.values(), file => file.path, options) }
export function listTexts(content: StoryContent, options: ContentPageOptions = {}) { return pageItems(content.texts.entries.values(), entry => entry.key, options) }
export function listLanguages(content: StoryContent, options: ContentPageOptions = {}) { return pageItems(content.texts.languages.values(), language => language.id, options) }
export function lookupTranslation(content: StoryContent, key: string, language: string): TranslationLookup {
...
export function listMissingTranslations(content: StoryContent, language: string, options: ContentPageOptions = {}) {
...
export function searchProgram(content: StoryContent, query: string, options: ContentPageOptions = {}): ContentPage<ProgramMatch> {
  queryText(query)
...
export function searchTexts(content: StoryContent, search: TextSearch, options: ContentPageOptions = {}): ContentPage<TextMatch> {

...
import type {
  StorySnapshot, DraftSnapshot, RevisionSnapshot, ContentOperation, StoryContent, ContentPageOptions, TextSearch,
...
export interface CreateScriptInput extends Omit<StorageCreate, 'initialContent'> {
  defaultLanguage: string
  contentMetadata?: JsonObject
}
export interface EditDraftInput { scriptId: ScriptId; expectedSequence: number; operations: readonly ContentOperation[]; metadata?: JsonObject }
export interface RestoreDraftInput { scriptId: ScriptId; expectedSequence: number; revisionId: RevisionId; selection: RestoreSelection }
export interface StoryCommitResult extends CommitResult { content: StoryContent }

export class StoryRepository {
  constructor(private readonly storage: StoryStorage) {}
  ...
  deleteScript(id: ScriptId): void { this.storage.deleteScript(id) }
  copyScript(input: CopyInput) {
    this.readSnapshot(input.source.kind === 'draft'
      ? { kind: 'draft', scriptId: input.sourceScriptId, sequence: input.source.expectedSequence }
      : { kind: 'revision', revisionId: input.source.revisionId })
    return this.storage.copyScript(input)
  }
  ...
  readSnapshot(ref: ContentRef): StorySnapshot
  readSnapshot(ref: ContentRef): StorySnapshot {
  ...
  private saveCandidate(before: DraftSnapshot, content: StoryContent, metadata?: JsonObject): DraftSnapshot {
    const input: DraftWrite = { scriptId: before.draft.scriptId, expectedSequence: before.draft.sequence,
  ...
  private savedSnapshot(draft: Draft, content: StoryContent): DraftSnapshot {
    return { kind: 'draft', ref: { kind: 'draft', scriptId: draft.scriptId, sequence: draft.sequence }, draft, content }
  }
  commitRevision(input: CommitInput): StoryCommitResult {
  ...
  registerPublication(input: { scriptId: ScriptId; revisionId: RevisionId; metadata?: JsonObject }) { return this.storage.createPublication(input) }
  getPublication(id: PublicationId) { return this.storage.getPublication(id) }
  listPublications(filter: { scriptId?: ScriptId; revisionId?: RevisionId }, options: PageOptions = {}) { return this.storage.listPublications(filter, options) }
  ...
  private pageSnapshot(ref: ContentRef, options: ContentPageOptions): StorySnapshot {

...
import type { ContentRef, RevisionId } from '@papermoon/story-storage'
...
function fixture() {
  ...
  const repository = new StoryRepository(storage), project = repository.createProject({ name: 'Scripts', metadata: { owner: 'local' } })
  const script = repository.createScript({ projectId: project.id, name: 'Empty', defaultLanguage: 'zh-CN', draftMetadata: { editor: 'draft' } })
  const ref = { kind: 'draft', scriptId: script.id } as const
  const current = () => repository.readSnapshot(ref)
  const seed = () => storage.writeDraft({ scriptId: script.id, expectedSequence: current().draft.sequence, changes: [...encodeContent(station())].map(([key, value]) => ({ kind: 'set', key, value })) })
  const commit = (description = 'Revision') => repository.commitRevision({ scriptId: script.id, expectedSequence: current().draft.sequence, description })
  const raw = () => { const db = new DatabaseSync(path); cleanups.push(() => db.close()); return db }
  return { path, storage, repository, project, script, ref, current, seed, commit, raw }
}
...
describe('persistent authoring', () => {
  it('creates a valid empty draft, commits incomplete content and registers publication without compilation', () => {
    ...
    f.repository.editDraft({ scriptId: f.script.id, expectedSequence: 1, operations: [{ kind: 'create-file', path: 'anything.ext', source: 'broken syntax {' }, { kind: 'create-text', key: 'untranslated' }] })
    const next = f.commit()
    expect(next.entry.ordinal).toBe(2)
    const publication = f.repository.registerPublication({ scriptId: f.script.id, revisionId: next.revision.id, metadata: { reason: 'internal' } })
    expect(f.repository.getPublication(publication.id)).toEqual(publication)
    expect(f.repository.registerPublication({ scriptId: f.script.id, revisionId: next.revision.id }).id).not.toBe(publication.id)
    expect(f.repository.listPublications({ scriptId: f.script.id }).items).toHaveLength(2)
    rejects('invalid-input', () => f.commit(' \n '))
    expect(f.repository.listRevisions(f.script.id).items).toHaveLength(2)
  ...
  it('writes a program/text batch once, preserves snapshots and rejects invalid final states atomically', () => {
    ...
    const changed = f.repository.editDraft({ scriptId: f.script.id, expectedSequence: 0, metadata: { editor: 'manual' }, operations: [
    ...
    rejects('invalid-content', () => f.repository.editDraft({ scriptId: f.script.id, expectedSequence: 1, metadata: {}, operations: [
      { kind: 'replace-file', path: 'main.js', source: 'bad candidate' }, { kind: 'delete-language', language: 'zh-CN' },
    ] }))
    expect(f.current()).toEqual(changed)
    const noop = f.repository.editDraft({ scriptId: f.script.id, expectedSequence: 1, operations: [] })
    expect(noop.draft.sequence).toBe(2)
    rejects('conflict', () => f.repository.editDraft({ scriptId: f.script.id, expectedSequence: 1, operations: [] }))
  })
  it('rejects an interleaved writer at the final CAS without saving its candidate', () => {
    const f = fixture(), second = new StoryStorage({ path: f.path }); cleanups.push(() => second.close())
    const original = f.storage.writeDraft.bind(f.storage)
    vi.spyOn(f.storage, 'writeDraft').mockImplementationOnce(input => {
      second.writeDraft({ scriptId: f.script.id, expectedSequence: 0, changes: [], metadata: { other: true } })
      return original(input)
    })
    rejects('conflict', () => f.repository.editDraft({ scriptId: f.script.id, expectedSequence: 0, operations: [{ kind: 'create-file', path: 'candidate', source: 'must not commit' }] }))
  ...
  it('keeps owner metadata and content in one read transaction under connection contention', () => {
    ...
    vi.spyOn(connection, 'all').mockImplementationOnce((...args) => {
      const rows = all(...args)
      rejects('busy', () => second.writeDraft({ scriptId: f.script.id, expectedSequence: 0, changes: [], metadata: { mixed: true } }))
      return rows
    })
    expect(f.current().draft).toMatchObject({ sequence: 0, metadata: { editor: 'draft' } })
    expect(second.getDraft(f.script.id).sequence).toBe(0)
  })
  it('keeps revision/directory metadata immutable and owner management changes out of content differences', () => {
    ...
    const first = f.repository.commitRevision({ scriptId: f.script.id, expectedSequence: 1, description: 'First', metadata: { revision: true }, historyMetadata: { directory: true } })
    f.repository.updateProject(f.project.id, { name: 'Renamed', metadata: { replaced: true } })
    f.repository.updateScript(f.script.id, { name: 'Renamed', metadata: { management: true } })
    f.repository.editDraft({ scriptId: f.script.id, expectedSequence: 2, metadata: { editor: 'changed' }, operations: [] })
    const next = f.commit('Second')
    expect(f.repository.getRevision(first.revision.id).metadata).toEqual({ revision: true })
    expect(f.repository.listRevisions(f.script.id).items[0]?.metadata).toEqual({ directory: true })
    ...
    expect(f.repository.getScript(f.script.id).metadata).toEqual({ management: true })
  })
  it('restores full or partial content with different provenance and without resetting draft metadata', () => {
    ...
    f.repository.editDraft({ scriptId: f.script.id, expectedSequence: 2, operations: [{ kind: 'replace-file', path: 'entry.mjs', source: 'changed' }, { kind: 'set-description', key: 'greeting', description: 'Changed purpose' }] })
    const second = f.commit()
    const partial = f.repository.restoreDraft({ scriptId: f.script.id, expectedSequence: 4, revisionId: first.revision.id, selection: { kind: 'text', key: 'greeting' } })
    ...
    const full = f.repository.restoreDraft({ scriptId: f.script.id, expectedSequence: 5, revisionId: first.revision.id, selection: { kind: 'all' } })
  ...
  it('copies history by reference, preserves independent publication and survives deleting the source', () => {
    ...
    f.repository.registerPublication({ scriptId: f.script.id, revisionId: first.revision.id, metadata: { one: 1 } })
    const project = f.repository.createProject({ name: 'Other' })
    const copy = f.repository.copyScript({ sourceScriptId: f.script.id, source: { kind: 'revision', revisionId: first.revision.id }, targetProjectId: project.id, name: 'Copy', history: 'copy', publications: 'copy' })
    expect(f.repository.listRevisions(copy.id).items.map(item => item.revision.id)).toEqual([first.revision.id])
    expect(f.repository.listPublications({ scriptId: copy.id }).items[0]?.metadata).toEqual({ one: 1 })
    ...
    rejects('not-found', () => f.repository.registerPublication({ scriptId: copy.id, revisionId: next.revision.id }))
    f.repository.deleteScript(copy.id)
  ...
  it('rejects malformed stored business content at the owning decoder', () => {
    const f = fixture()
    f.storage.writeDraft({ scriptId: f.script.id, expectedSequence: 0, changes: [{ kind: 'delete', key: 'content' }] })
  ...
  it('retains results after reopening and exposes the storage closed failure', () => {
    ...
    expect(new StoryRepository(reopened).readSnapshot({ kind: 'revision', revisionId: revision.revision.id }).content).toEqual(revision.content)
  })
})
...
describe('business queries and SQL comparison', () => {
  it('groups multiple translation changes with source and metadata differences', () => {
    const f = fixture(); f.seed(); const revision = f.commit()
    f.repository.editDraft({ scriptId: f.script.id, expectedSequence: 2, operations: [
  ...
  it('pins list, search, missing-translation pages and subsequent detail reads', () => {
    ...
    f.repository.editDraft({ scriptId: f.script.id, expectedSequence: 1, operations: [] })
  ...
  it('compares two scripts and rejects later draft pages after either side changes', () => {
    const f = fixture(); f.seed()
    const second = f.storage.createScript({ projectId: f.project.id, name: 'Workshop', initialContent: encodeContent(workshop()) })
    const right: ContentRef = { kind: 'draft', scriptId: second.id }
    const diff = f.repository.compare(f.ref, right, { limit: 1 })
    expect(diff.next).toBeDefined()
    f.repository.editDraft({ scriptId: second.id, expectedSequence: 0, operations: [] })

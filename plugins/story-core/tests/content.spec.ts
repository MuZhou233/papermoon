...
import type { JsonValue, MetadataTarget, StoryContent } from '../src/index.ts'
...
describe('authored content', () => {
  ...
  it('requires known formats and complete records rather than repairing data', () => {
    ...
    const header = records.get('content') as Record<string, JsonValue>
    for (const invalid of [new Map(), new Map([...records].filter(([key]) => key !== 'content')),
      new Map(records).set('content', { ...header, version: 99 }), new Map(records).set('unexpected', {}),
  ...
  it('validates metadata JSON without evaluating accessors or accepting lossy values', () => {
    ...
    for (const metadata of [NaN, [], { value: undefined }, accessor, cycle]) {
      rejects('invalid-content', () => createContent({ defaultLanguage: 'x', metadata: metadata as StoryContent['metadata'] }))
    }

...
import type { NativeTool } from '../../playbook-tools/src/plugin.ts'
const cleanup: (() => void)[] = []
afterEach(() => { for (const dispose of cleanup.splice(0).reverse()) dispose() })
function setup() {
  const dir = mkdtempSync(join(tmpdir(), 'papermoon-writer-'))
  const storage = new PlaybookStorage({ path: join(dir, 'playbook.sqlite') }), core = new PlaybookRepository(storage)
  ...
  async function select(target: Agent) { service.prepare(target, playbook.id); await service.configure({ sessionId: target.id, writerId: writer.id, writerSequence: writer.sequence }) }
  ...
  return { core, playbook, writer, writers, service, agent, select, accept, message, host }
}
...
test('missing selections and changed writer versions refuse admission without freezing', async () => {
  ...
  expect(() => h.service.admit(a.agent, h.message)).toThrow(/playbook workspace/)
  h.service.prepare(a.agent, h.playbook.id)
...
test('scopes share a playbook without sharing observations and remounts require a new read', async () => {
  const h = setup(), a = h.agent('a'), b = h.agent('b'); await h.select(a.agent); await h.select(b.agent)
  h.core.editDraft({ playbookId: h.playbook.id, expectedSequence: 0, operations: [{ kind: 'create-file', path: 'main.js', source: 'first' }] })
...
test('deleted targets retain history and refuse messages and tool execution', async () => {
  const h = setup(), a = h.agent('a'); await h.select(a.agent); h.accept(a.agent)
  h.core.deletePlaybook(h.playbook.id)
  expect(h.service.view(a.agent).targetMissing).toBe(true)
  expect(() => h.service.admit(a.agent, h.message)).toThrow(/no longer exists/)
  await expect(a.definitions.get('playbook_status')!.execute({}, { callId: 'test-call', signal: new AbortController().signal })).rejects.toThrow(/no longer exists/)
...
test('workspace labels use the playbook name while folder and deleted targets retain their labels', async () => {
  const h = setup()
  const workspace = (id: string, title: string, key?: string) => ({
    id, title, path: '/fixture', sessionIds: [],
    ...(key ? { target: { provider: 'papermoon-playbook', key } } : {}),
    async setTitle(value: string) { this.title = value },
    async attachSession() {}, async detachSession() {},
  })
  const playbook = workspace('playbook', 'P / S', h.playbook.id), folder = workspace('folder', 'Folder'), deleted = workspace('deleted', 'Removed playbook', 'gone')
  h.host.workspaceRegistry = {
    list: () => [playbook, folder, deleted], createResource: async () => playbook,

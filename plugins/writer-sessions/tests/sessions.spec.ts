...
test('scopes share a script without sharing observations and remounts require a new read', async () => {
  ...
  await expect(call(b, 'story_program_edit', { expectedSequence: 1, operations: [{ kind: 'replace-file', path: 'main.js', source: 'second' }] })).rejects.toThrow(/not-observed/)
  await call(a, 'story_program_edit', { expectedSequence: 1, operations: [{ kind: 'replace-file', path: 'main.js', source: 'second' }] })
  a.dispose(); expect(a.definitions.size).toBe(0); expect(b.definitions.size).toBe(14)
  const resumed = h.agent('a', a.events)
  await expect(call(resumed, 'story_program_edit', { expectedSequence: 2, operations: [{ kind: 'delete-file', path: 'main.js' }] })).rejects.toThrow(/not-observed/)
})

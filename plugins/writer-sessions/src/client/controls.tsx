...
export function ScriptPicker({ runtime, t, folderPicker, pickerOpen: open, setPickerOpen: setOpen, selectWorkspace }: Props & { folderPicker: ReactNode; pickerOpen: boolean; setPickerOpen(open: boolean): void; selectWorkspace(id: string): Promise<void> }) {
  ...
  return <>
    <button type="button" className="pws-chip" disabled={!!state.view?.fixed || busy || mode.busy} onClick={() => setOpen(true)}>
      <BookIcon size={16} /><span>{state.view?.target?.name ?? t('script')}</span><span aria-hidden="true">⌄</span>
    </button>
    <Modal closeLabel={t('close')} open={open} onClose={() => { if (!busy) setOpen(false) }} title={t('script')}>
      <div className="pws-picker">
        ...
        <div className="pws-script-list">{rows.map(row => <button key={row.scriptId} type="button" disabled={busy} className="pws-script" onClick={() => {
          setBusy(true); setError('')
          void runtime.workspace(row.scriptId).then(selectWorkspace).then(() => { setOpen(false); void runtime.refresh() }).catch(error => setError(String(error))).finally(() => setBusy(false))
        }}><BookIcon size={18} /><span><strong>{row.scriptName}</strong><small>{row.projectName}</small></span></button>)}{rows.length === 0 && <p>{t('empty')}</p>}</div>

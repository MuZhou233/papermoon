...
import { StoryStorage } from '../lib/index.js'
...
acceptsHost(root)
try {
  const config = { path: join(directory, 'story.sqlite') }
  let provider = await root.plugin(contract, config)
  const storage = root.get(adapter.serviceKey) as StoryStorage
  assert.ok(storage instanceof StoryStorage)
  ...
  const consumer = await root.plugin({
    ...
    apply(ctx: HostContext) {
      const service = ctx.get(adapter.serviceKey) as StoryStorage
      ctx.effect(() => async () => {
  ...
  const reopened = root.get(adapter.serviceKey) as StoryStorage

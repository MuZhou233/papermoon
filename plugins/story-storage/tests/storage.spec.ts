...
import { StoryStorage, StorageError, STORAGE_APPLICATION_ID, STORAGE_FORMAT_VERSION } from '../src/index.ts'
...
function fixture() {
  ...
  const store = new StoryStorage({ path }); cleanups.push(() => store.close())
  const project = store.createProject({ name: 'Station' })
  const script = store.createScript({ projectId: project.id, name: 'Cooperation' })
  const raw = () => { const db = new DatabaseSync(path); cleanups.push(() => db.close()); return db }
  const put = (values: Record<string, JsonValue>) => store.writeDraft({ scriptId: script.id, expectedSequence: store.getDraft(script.id).sequence, changes: Object.entries(values).map(([key, value]) => ({ kind: 'set', key, value })) })
  const commit = (description = '') => store.commitRevision({ scriptId: script.id, expectedSequence: store.getDraft(script.id).sequence, description })
  return { directory, path, store, project, script, raw, put, commit }
}
...
describe('drafts and immutable revisions', () => {
  it('stores code and localized copy as ordinary keys, including unfinished source', () => {
    ...
    const read = f.store.readContent({ kind: 'draft', scriptId: f.script.id, sequence: draft.sequence })
    ...
    expect(committed.draft).toEqual({ scriptId: f.script.id, sequence: 2, baseRevisionId: committed.revision.id, metadata: {} })
    ...
    f.store.restoreDraft({ scriptId: f.script.id, expectedSequence: second.draft.sequence, revisionId: committed.revision.id })
    ...
    expect(f.store.listRevisions(f.script.id).items.map(x => x.revision.id)).toEqual([committed.revision.id, second.revision.id, third.revision.id])
  ...
  it('does not give callers a mutable alias and distinguishes missing from null', () => {
    ...
    const ref: ContentRef = { kind: 'draft', scriptId: f.script.id }
    ...
    const draft = f.store.getDraft(f.script.id)
    f.store.writeDraft({ scriptId: f.script.id, expectedSequence: draft.sequence, changes: [{ kind: 'set', key: 'nullable', value: 1 }, { kind: 'delete', key: 'nullable' }] })
  ...
  it('keeps identity and historical provenance across renames and uses metadata replacement', () => {
    ...
    const script = f.store.updateScript(f.script.id, { name: 'Reworked', metadata: { role: 'candidate' } })
    expect(project.id).toBe(f.project.id); expect(script.id).toBe(f.script.id)
    expect(f.store.getRevision(first.revision.id).source.projectName).toBe('Station')
    expect(f.store.getRevision(first.revision.id).source.scriptName).toBe('Cooperation')
    expect(f.store.updateProject(project.id, { metadata: {} }).metadata).toEqual({})
    expect(f.store.updateScript(script.id, { metadata: {} }).metadata).toEqual({})
  ...
  it('paginates metadata without embedding revision content', () => {
    ...
    const first = f.store.listRevisions(f.script.id, { limit: 2 })
    expect(first.items.map(x => x.ordinal)).toEqual([1, 2]); expect(first.next).toBe(2)
    const second = f.store.listRevisions(f.script.id, { limit: 2, after: first.next })
    ...
    f.store.createScript({ projectId: f.project.id, name: 'Other' })
    const scripts = f.store.listScripts(f.project.id, { limit: 1 })
    expect(f.store.listScripts(f.project.id, { limit: 1, after: scripts.next }).items).toHaveLength(1)
...
describe('copy, publication and retention', () => {
  it('copies reference prefixes and independent drafts without duplicating revision bodies', () => {
    ...
    const published = f.store.createPublication({ scriptId: f.script.id, revisionId: first.revision.id, metadata: { release: 'A' } })
    f.store.createPublication({ scriptId: f.script.id, revisionId: second.revision.id })
    const raw = f.raw()
    ...
    const copy = f.store.copyScript({ sourceScriptId: f.script.id, source: { kind: 'revision', revisionId: first.revision.id }, targetProjectId: targetProject.id, name: 'Alternative', history: 'copy', publications: 'copy' })
    ...
    expect(f.store.readEntry({ kind: 'draft', scriptId: copy.id }, 'value').entry).toEqual({ exists: true, value: 'first' })
    const copiedPublication = f.store.listPublications({ scriptId: copy.id }).items[0]!
    ...
    f.store.writeDraft({ scriptId: copy.id, expectedSequence: 0, changes: [{ kind: 'set', key: 'value', value: 'independent' }] })
    const copiedCommit = f.store.commitRevision({ scriptId: copy.id, expectedSequence: 1, description: 'new direction' })
    expect(copiedCommit.entry.ordinal).toBe(2)
    expect(f.store.listRevisions(f.script.id).items[1]?.revision.id).toBe(second.revision.id)
    f.store.deleteProject(f.project.id)
    expect(f.store.getRevision(first.revision.id).source.scriptId).toBe(f.script.id)
    expect(f.store.getPublication(copiedPublication.id).scriptId).toBe(copy.id)
  ...
  it('copies current drafts with explicit scopes and preserves source sequence', () => {
    ...
    f.put({ value: 2 }); const draft = f.store.getDraft(f.script.id)
    const base = { sourceScriptId: f.script.id, source: { kind: 'draft' as const, expectedSequence: draft.sequence }, targetProjectId: f.project.id, name: 'Copy' }
    const full = f.store.copyScript({ ...base, history: 'copy', publications: 'none' })
    const onlyContent = f.store.copyScript({ ...base, history: 'none', publications: 'none' })
    expect(f.store.getDraft(f.script.id)).toEqual(draft)
    ...
    expect(f.store.readEntry({ kind: 'draft', scriptId: onlyContent.id }, 'value').entry).toEqual({ exists: true, value: 2 })
    expect(f.store.commitRevision({ scriptId: onlyContent.id, expectedSequence: 0, description: '' }).entry.ordinal).toBe(1)
    fails('invalid-input', () => f.store.copyScript({ ...base, history: 'none', publications: 'copy' }))
    fails('conflict', () => f.store.copyScript({ ...base, source: { kind: 'draft', expectedSequence: 0 }, history: 'copy', publications: 'none' }))
    fails('not-found', () => f.store.copyScript({ ...base, sourceScriptId: onlyContent.id, source: { kind: 'revision', revisionId: original.revision.id }, history: 'copy', publications: 'none' }))
  })
  it('requires local history for publication and does not retain informational references', () => {
    const f = fixture(); f.put({ value: 1 }); const original = f.commit()
    const other = f.store.createScript({ projectId: f.project.id, name: 'Unrelated' })
    fails('not-found', () => f.store.createPublication({ scriptId: other.id, revisionId: original.revision.id }))
    f.store.restoreDraft({ scriptId: other.id, expectedSequence: 0, revisionId: original.revision.id })
    expect(f.store.listRevisions(other.id).items).toEqual([])
    const next = f.store.commitRevision({ scriptId: other.id, expectedSequence: 1, description: '', references: [original.revision.id] })
    const publications = [f.store.createPublication({ scriptId: other.id, revisionId: next.revision.id }), f.store.createPublication({ scriptId: other.id, revisionId: next.revision.id })]
    const first = f.store.listPublications({ revisionId: next.revision.id }, { limit: 1 })
    expect(f.store.listPublications({ scriptId: other.id }, { limit: 1, after: first.next }).items).toHaveLength(1)
    ...
    expect(f.store.readEntry({ kind: 'draft', scriptId: other.id }, 'value').entry).toEqual({ exists: true, value: 1 })
  })
})
...
describe('database-side differences', () => {
  it('returns values and presence for all change kinds, with stable key ranges', () => {
    ...
    f.store.writeDraft({ scriptId: f.script.id, expectedSequence: first.draft.sequence, changes: [
      { kind: 'set', key: 'same', value: { b: 2, a: 1 } }, { kind: 'set', key: 'changed', value: '1' }, { kind: 'set', key: 'added', value: null }, { kind: 'delete', key: 'removed' },
    ] })
    const diff = f.store.compare({ kind: 'revision', revisionId: first.revision.id }, { kind: 'draft', scriptId: f.script.id }, { limit: 2 })
    ...
    fails('invalid-input', () => f.store.compare(diff.left, { kind: 'draft', scriptId: f.script.id }, { after: diff.next }))
    fails('invalid-input', () => f.store.listEntries({ kind: 'draft', scriptId: f.script.id }, { after: 'changed' }))
  ...
  it('compares cross-script drafts/revisions without changing drafts or history', () => {
    const f = fixture(); f.put({ value: 1 }); const first = f.commit()
    const other = f.store.createScript({ projectId: f.project.id, name: 'Other' })
    f.store.writeDraft({ scriptId: other.id, expectedSequence: 0, changes: [{ kind: 'set', key: 'value', value: 2 }] })
    expect(f.store.compare({ kind: 'draft', scriptId: f.script.id }, { kind: 'draft', scriptId: other.id }).items[0]?.kind).toBe('modified')
    const second = f.store.commitRevision({ scriptId: other.id, expectedSequence: 1, description: '' })
    ...
    expect(f.store.getDraft(f.script.id)).toEqual(first.draft)
...
describe('atomicity, format and lifecycle', () => {
  ...
  it('validates an entire batch before mutation and preserves unusual valid keys', () => {
    const f = fixture(); const draft = f.store.getDraft(f.script.id)
    fails('invalid-input', () => f.store.writeDraft({ scriptId: f.script.id, expectedSequence: draft.sequence, changes: [
    ...
    expect(f.store.readEntry({ kind: 'draft', scriptId: f.script.id }, 'a\\0b').entry).toEqual({ exists: true, value: 'value' + String.fromCharCode(0) })
    const onlyContent = f.store.copyScript({ sourceScriptId: f.script.id, source: { kind: 'revision', revisionId: f.commit().revision.id }, targetProjectId: f.project.id, name: 'Content', history: 'none', publications: 'none' })
    expect(f.store.listRevisions(onlyContent.id).items).toEqual([])
    expect(f.store.readContent({ kind: 'draft', scriptId: onlyContent.id }).content.size).toBe(3)
  })
  ...
  it('rolls back a partial batch and a partially copied revision', () => {
    ...
    const draft = f.store.getDraft(f.script.id)
    fails('database-error', () => f.store.writeDraft({ scriptId: f.script.id, expectedSequence: draft.sequence, changes: [{ kind: 'set', key: 'a', value: 3 }, { kind: 'set', key: 'fail', value: true }] }))
    expect(f.store.getDraft(f.script.id)).toEqual(draft)
    expect(f.store.readEntry({ kind: 'draft', scriptId: f.script.id }, 'a').entry).toEqual({ exists: true, value: 1 })
  ...
  it('rolls back a copy if its publication fails after draft and history insertion', () => {
    ...
    f.store.createPublication({ scriptId: f.script.id, revisionId: first.revision.id })
    const raw = f.raw(); const scripts = f.store.listScripts(f.project.id)
    raw.exec("CREATE TRIGGER fail_publication BEFORE INSERT ON publications BEGIN SELECT RAISE(ABORT,'fixture'); END")
    fails('database-error', () => f.store.copyScript({ sourceScriptId: f.script.id, source: { kind: 'revision', revisionId: first.revision.id }, targetProjectId: f.project.id, name: 'Copy', history: 'copy', publications: 'copy' }))
    expect(f.store.listScripts(f.project.id)).toEqual(scripts)
  ...
  it('detects stale writers across independent connections and reports SQLite contention', () => {
    const f = fixture(); const second = new StoryStorage({ path: f.path }); cleanups.push(() => second.close())
    const old = second.getDraft(f.script.id)
    f.put({ value: 1 })
    fails('conflict', () => second.writeDraft({ scriptId: f.script.id, expectedSequence: old.sequence, changes: [] }))
    fails('conflict', () => second.commitRevision({ scriptId: f.script.id, expectedSequence: old.sequence, description: '' }))
    fails('conflict', () => second.restoreDraft({ scriptId: f.script.id, expectedSequence: old.sequence, revisionId: 'missing' as RevisionId }))
    expect(second.readEntry({ kind: 'draft', scriptId: f.script.id }, 'value').entry).toEqual({ exists: true, value: 1 })
    const raw = f.raw(); raw.exec('BEGIN IMMEDIATE')
    try { fails('busy', () => f.put({ value: 2 })) } finally { raw.exec('ROLLBACK') }
    const beforeCommit = f.store.getDraft(f.script.id)
    raw.exec('BEGIN'); raw.prepare('SELECT * FROM draft_entries').all()
    try { fails('busy', () => f.put({ value: 2 })) } finally { raw.exec('ROLLBACK') }
    expect(f.store.getDraft(f.script.id)).toEqual(beforeCommit)
    expect(second.readEntry({ kind: 'draft', scriptId: f.script.id }, 'value').entry).toEqual({ exists: true, value: 1 })
    f.put({ value: 3 })
    expect(second.readEntry({ kind: 'draft', scriptId: f.script.id }, 'value').entry).toEqual({ exists: true, value: 3 })
  })
  it('reopens durable data and refuses foreign, changed or damaged formats without replacement', () => {
    ...
    const reopened = new StoryStorage({ path: f.path }); reopened.close()
    const raw = f.raw()
    expect(raw.prepare('PRAGMA application_id').get()).toMatchObject({ application_id: STORAGE_APPLICATION_ID })
    expect(raw.prepare('PRAGMA user_version').get()).toMatchObject({ user_version: STORAGE_FORMAT_VERSION })
    ...
    raw.exec(`PRAGMA user_version=${STORAGE_FORMAT_VERSION}`)
    const valid = new StoryStorage({ path: f.path }); cleanups.push(() => valid.close())
    ...
    fails('format-mismatch', () => new StoryStorage({ path: foreignPath })); expect(readFileSync(foreignPath)).toEqual(foreignBytes)
    const damagedPath = join(f.directory, 'damaged.sqlite'); writeFileSync(damagedPath, 'not a database')
    fails('corrupt', () => new StoryStorage({ path: damagedPath })); expect(readFileSync(damagedPath, 'utf8')).toBe('not a database')
  ...
  it('recovers committed data and discards an interrupted transaction after process exit', () => {
    ...
    const reopened = new StoryStorage({ path: f.path }); cleanups.push(() => reopened.close())
    expect(reopened.readEntry({ kind: 'draft', scriptId: f.script.id }, 'value').entry).toEqual({ exists: true, value: 'saved' })
    expect(reopened.listRevisions(f.script.id).items).toHaveLength(1)
  })
  it('unregisters before closing and releases a database after registration failure', async () => {
    const f = fixture(); f.store.close()
    let stored: StoryStorage | undefined; let dispose: (() => void | Promise<void>) | undefined
    ...
    let failedHandle: StoryStorage | undefined
...
describe('owner metadata and atomic snapshots', () => {
  it('creates initial values and draft metadata at sequence zero and freezes directory metadata', () => {
    const f = fixture()
    const script = f.store.createScript({ projectId: f.project.id, name: 'Initialized', initialContent: new Map([['initial', { ready: true }]]), draftMetadata: { editor: 'local' } })
    const snapshot = f.store.readSnapshot({ kind: 'draft', scriptId: script.id })
    ...
    const committed = f.store.commitRevision({ scriptId: script.id, expectedSequence: 0, description: '', metadata: { commit: true }, historyMetadata: { catalog: true } })
    ...
    expect(f.store.listRevisions(script.id).items[0]?.metadata).toEqual({ catalog: true })
    ...
    const copy = f.store.copyScript({ sourceScriptId: script.id, source: { kind: 'revision', revisionId: committed.revision.id }, targetProjectId: f.project.id, name: 'History', history: 'copy', publications: 'none' })
    ...
    const fromDraft = f.store.copyScript({ sourceScriptId: script.id, source: { kind: 'draft', expectedSequence: 1 }, targetProjectId: f.project.id, name: 'Working', history: 'none', publications: 'none' })
    expect(f.store.getDraft(fromDraft.id).metadata).toEqual({ editor: 'local' })
    const override = f.store.copyScript({ sourceScriptId: script.id, source: { kind: 'draft', expectedSequence: 1 }, targetProjectId: f.project.id, name: 'Override', history: 'copy', publications: 'none', draftMetadata: { own: true } })
  ...
  it('replaces draft metadata with CAS and preserves it when restoring content', () => {
    ...
    const draft = f.store.writeDraft({ scriptId: f.script.id, expectedSequence: revision.draft.sequence, changes: [{ kind: 'set', key: 'value', value: 2 }], metadata: { local: true } })
    expect(draft.metadata).toEqual({ local: true })
    fails('conflict', () => f.store.writeDraft({ scriptId: f.script.id, expectedSequence: revision.draft.sequence, changes: [], metadata: {} }))
    const restored = f.store.restoreDraft({ scriptId: f.script.id, expectedSequence: draft.sequence, revisionId: revision.revision.id })
    expect(restored.metadata).toEqual({ local: true })
    expect(f.store.writeDraft({ scriptId: f.script.id, expectedSequence: restored.sequence, changes: [], metadata: {} }).metadata).toEqual({})
  })
  it('rolls back initialization and content changes when owner writes fail', () => {
    ...
    fails('database-error', () => f.store.createScript({ projectId: f.project.id, name: 'Failed', draftMetadata: { initialized: true }, initialContent: new Map([['first', 1], ['fail', 2]]) }))
    expect(f.store.listScripts(f.project.id).items).toHaveLength(1)
    expect(raw.prepare('SELECT COUNT(*) AS n FROM drafts').get()).toMatchObject({ n: 1 })
    raw.exec("CREATE TRIGGER reject_metadata BEFORE UPDATE ON drafts BEGIN SELECT RAISE(ABORT,'fixture'); END")
    fails('database-error', () => f.store.writeDraft({ scriptId: f.script.id, expectedSequence: 0, metadata: { changed: true }, changes: [{ kind: 'set', key: 'value', value: 1 }] }))

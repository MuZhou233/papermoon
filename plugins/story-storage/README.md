...
A project contains independent scripts. Each script owns one draft and an ordered history directory. Directory entries reference immutable revisions; copying history copies these references, not revision bodies. Publication records belong to a script and a revision in that script's directory. They contain no approval workflow or compiled artifact.

Content is a map from strings to JSON values. Draft and revision entries occupy separate SQLite tables, with one row per top-level key. Objects inside a value remain JSON. The module does not interpret keys as paths, code, languages or copy. Consumers own those conventions and their business format versions.
...
Deleting a script removes its draft, history references and publications. Deleting a project cascades through its scripts. Unreferenced revisions and their entries are removed in the same transaction; another script's history keeps its shared revisions alive. There is no individual history deletion API, automatic history trimming or content deduplication between independently created revisions.

## API and pagination

The [public types](src/types.ts) describe requests and results; [StoryStorage](src/storage.ts) implements synchronous operations. Failures use [StorageError](src/error.ts) codes: invalid-input, not-found, conflict, busy, closed, format-mismatch, corrupt and database-error. A call returns after its transaction commits. Calls after close fail; close itself is idempotent.
...
The [storage decision](../../.agents/notes/implemented/architecture/2026-09-12-script-storage.md) records the alternatives and ownership choices. Main and integration checks follow the [testing guide](../../docs/testing.md).

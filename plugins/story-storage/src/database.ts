...
const definitions = [
  ...
  `CREATE TABLE drafts (script_id TEXT PRIMARY KEY REFERENCES scripts(id) ON DELETE CASCADE, sequence INTEGER NOT NULL CHECK(sequence >= 0 AND sequence <= 9007199254740991), base_revision_id TEXT) STRICT`,
  ...
  `CREATE TABLE script_revisions (script_id TEXT NOT NULL REFERENCES scripts(id) ON DELETE CASCADE, ordinal INTEGER NOT NULL CHECK(ordinal > 0 AND ordinal <= 9007199254740991), revision_id TEXT NOT NULL REFERENCES revisions(id), PRIMARY KEY(script_id, ordinal), UNIQUE(script_id, revision_id)) STRICT`,

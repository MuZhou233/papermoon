...
export interface Config { directory: string; limits?: Partial<Limits> }
...
export function apply(ctx: Host, config: Config): void {
  ...
  ctx.effect(function* () {
    const service = new CompilationService(ctx.get('papermoonStoryCore') as StoryRepository, new ArtifactStore(config.directory), { limits: config.limits })
    yield () => service.close()

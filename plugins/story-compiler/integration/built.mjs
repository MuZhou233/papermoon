...
import { createContent, applyOperations } from '../../story-core/lib/index.js'
import { compile } from '../lib/index.js'
const content = applyOperations(createContent({ defaultLanguage: 'en' }), [
  { kind: 'create-file', path: 'story.js', source: 'module.exports={systemPrompt:"{{literal}}",messages:[{role:"assistant",content:"Opening"}]}' },
])
...
const child = spawnSync(process.execPath, ['--input-type=module', '-e', `
  ...
  registerHooks({ resolve(specifier, context, next) {
    if (['node:fs', 'node:fs/promises', 'node:worker_threads', 'node:sqlite'].includes(specifier) || specifier.includes('story-core') || specifier.includes('cordis')) throw new Error('runtime imported ' + specifier);

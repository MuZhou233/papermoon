...
剧本存储要求格式 3。版本 1 和 2 数据库会被拒绝，原文件保持不变。使用新存储前，将旧文件保存在配置的新数据库路径之外，不提供迁移或自动删除。业务 KV 和开场产物格式仍为版本 1。compiled-stories 目录只保存独立草稿检查，不再需要这些预览时可删除；权威修订产物保存在 Story SQLite 中，已经开始的演绎另有日志副本。

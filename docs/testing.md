...
Writer unit tests cover settings, literal context and unsaved decisions. Playbook-tool tests exercise temporary repositories; the real DSH check additionally verifies scope isolation and tool output validation. Browser scenarios cover prompt ordering, readonly catalog presentation, navigation protection and conflicting saves. Main checks remain independent of the managed checkout.
...
Compiler tests cover restricted modules, literal messages, translation failures, deadlines, cancellation, fixed snapshots and immutable artifact writes. Main CI also runs `pnpm check:compiler:built` without DSH. The compiler directory is included in integration path filters. Real tool and writer-runtime checks include asynchronous `playbook_compile` results; browser scenarios cover save confirmation, diagnostic navigation, source changes and artifact recovery. The built check reports the local Node version; passing locally does not claim a CI result.
...
Story Mode checks cover attempt gates, requirement-owned copy and advisory effort rules in the main suite. `pnpm test:story-mode` uses a real hosted DSH application and deterministic adapter for Chapter 0, including configuration recovery, invalid defaults, continued conversation and visibility of the first successful reply's trajectory. The text-conversation integration checks exact requests, original request inspection, the first successful turn, persistence and cancellation against managed DSH builds.

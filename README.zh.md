...
[人工编辑器](plugins/story-editor/README.zh.md)提供剧本总览、程序与多语言文案编辑、不可变修订版本、比较、复制和恢复。编辑器通过[逻辑核心](plugins/story-core/README.zh.md)使用独立的[存储模块](plugins/story-storage/README.zh.md)。编译和模型驱动的创作尚未提供。
...
DSH 运行数据默认位于 `.papermoon/dsh` 和 `.papermoon/agents`。显式设置 `DSH_HOME` 和 `DSH_AGENTS_HOME` 可覆盖路径。剧本默认存于 `.papermoon/story.sqlite`，`PAPERMOON_DATA_DIR` 可覆盖其父目录。凭据由 DSH 按原有环境和配置机制解析；根目录 `.env` 被 Git 忽略。默认会话工作区是 PaperMoon 根目录。
